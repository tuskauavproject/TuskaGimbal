...
void mixTable() {
 ...
 /************************************************************************************************************/ 
   ...
   // Simple CameraGimbal By Bledy http://youtu.be/zKGr6iR54vM
    if (rcOptions[BOXCAMSTAB]) {
      if(0)
      {
        servo[0] = constrain(rcData[AUX1] - (-TILT_ROLL_PROP) * angle[PITCH] /16 - TILT_ROLL_PROP * angle[ROLL] /16 , TILT_PITCH_MIN, TILT_PITCH_MAX);
        servo[1] = constrain(rcData[AUX2] + (-TILT_ROLL_PROP) * angle[PITCH] /16 - TILT_ROLL_PROP * angle[ROLL] /16 , TILT_ROLL_MIN, TILT_ROLL_MAX);
      }
      else if(1)
      {
 ...
 /************************************************************************************************************/ 
  ...
  #ifdef GIMBAL
    if(rcData[YAW] <= 1300 && !(failsafeCnt > (5*FAILSAVE_DELAY)))
    {
      servo[0] = constrain(TILT_PITCH_MIDDLE + TILT_PITCH_PROP * angle[PITCH] /16 + map(rcData[AUX1], 1000, 2000, -500, 500), TILT_PITCH_MIN, TILT_PITCH_MAX);
      servo[1] = constrain(TILT_ROLL_MIDDLE + TILT_ROLL_PROP   * angle[ROLL]  /16 + map(rcData[AUX2], 1000, 2000, -500, 500), TILT_ROLL_MIN, TILT_ROLL_MAX);
    }
    else if(rcData[YAW] > 1300 & rcData[YAW] < 1800 && !(failsafeCnt > (5*FAILSAVE_DELAY)))
    {
      servo[0] = constrain(TILT_PITCH_MIDDLE + TILT_PITCH_PROP * angle[PITCH] /16, TILT_PITCH_MIN, TILT_PITCH_MAX);
      servo[1] = constrain(TILT_ROLL_MIDDLE + TILT_ROLL_PROP   * angle[ROLL]  /16, TILT_ROLL_MIN, TILT_ROLL_MAX);
    }
    else
    {
      servo[0] = constrain(TILT_PITCH_MIDDLE, TILT_PITCH_MIN, TILT_PITCH_MAX);
      servo[1] = constrain(TILT_ROLL_MIDDLE, TILT_ROLL_MIN, TILT_ROLL_MAX);
    }
